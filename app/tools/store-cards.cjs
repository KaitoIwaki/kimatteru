...
const CARDS = [
  // 1. 看板。App名「未定のまま置けるカレンダー」と同じことを言う
  { file: 'card-1', src: 'ss-1-month.png',
    eyebrow: '決まった予定は塗り、まだの予定は点線',
    lines: [[['', '未定', 'のまま、'], false], [['置ける。', '', ''], null]], size: 34 },
  // 2. 見出しは画面に写っている字をなぞらない。ダイアログの文言は絵の中に出ているので、
  //    ここでは**何が起きるか**を言う（点線が塗りに変わる）
  { file: 'card-2', src: 'ss-6-dialog.png',
    lines: [[['点線が、塗りに変わる。', '', ''], null], [['聞かれて、選ぶだけ。', '', ''], null]], size: 30 },
  // 3. ウィジェット。**本物のホーム画面のスクリーンショット**（実機で撮ったもの）を使う。
  //    描いたウィジェットは使わない（実物と違うものになる）。無ければこの枚は飛ばす
  { file: 'card-3', src: 'ss-widget.png',
    lines: [[['ホーム画面に、', '', ''], null], [['', 'まだ', '何件か。'], false]], size: 32 },
  // 4. 空きを画像1枚で渡す。他のカレンダーに無いもの
  { file: 'card-4', src: 'ss-4-share.png',
    lines: [[['「いつ空いてる？」に、', '', ''], null], [['画像1枚で答える。', '', ''], null]], size: 28 },
  // 5. まとめ。1.2 から「何に時間を使ったか」。バイトの人だけのものではなくなった
  { file: 'card-5', src: 'ss-5-report.png',
    lines: [[['何に時間を', '', ''], null], [['使ったか、見える。', '', ''], null]], size: 30 },
].filter((c) => {
  const ok = require('node:fs').existsSync(join(SRC, c.src));
  if (!ok) console.log('★ 無いので飛ばす:', c.src, '（', c.file, '）');
  return ok;
});
