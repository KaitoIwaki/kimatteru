...
// 地：上が白に近く、下がわずかに灰の生成り。minical は灰、こちらはアプリの地に合わせて少し暖かく
async function background() {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}">
    <defs><linearGradient id="g" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#FBFAF7"/><stop offset="1" stop-color="#E8E6DF"/></linearGradient></defs>
...
async function callout(canvas, shotFile, crop, g, scale, dy = 90) {
  ...
  const left = Math.round(Math.min(Math.max(40, W / 2 - w / 2), W - 40 - w));
...
async function card(n, { sub, lines, accent, shot, callouts = [], tap = null, screen = null }) {
  let canvas = await background();
  const t = await drawTitle(canvas, W, H, sub, lines, accent);
  canvas = t.canvas;
  const top = t.bottom + 70;
  ...
  for (const c of callouts) canvas = await callout(canvas, shotFile, c.crop, g, c.scale ?? 0.9, c.dy ?? 90);
...
async function homeScreen() {
  ...
  const wall = `<defs><linearGradient id="w" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#DDE8D9"/><stop offset="1" stop-color="#F1ECE2"/></linearGradient></defs><rect width="${sw}" height="${sh}" fill="url(#w)"/>`;
...
await card(1, { sub: 'たぶんの予定も、そのまま', lines: ['未定のまま、', '置ける'], accent: ['未定'], shot: '1-calendar.png',
  callouts: [{ crop: [0, 1258, 1290, 180], scale: 0.9, dy: 64 }] });   // 13〜19 日：塗りと点線が並ぶ行
...
await card(4, { sub: '○△× で、空きがひと目', lines: ['いつ空いてる？', 'すぐ答える'], accent: ['すぐ答える'], shot: '3-free.png',
  callouts: [{ crop: [40, 838, 1210, 150], scale: 0.9, dy: 64 }] });    // 木 3：バイトのあと空いてる行
await card(5, { sub: 'バイトも、遊びも、用事も', lines: ['何に時間を', '使ったか、見える'], accent: ['見える'], shot: '5-report.png',
  callouts: [{ crop: [50, 700, 1190, 520], scale: 0.9, dy: 64 }] });    // 62 時間と内訳
