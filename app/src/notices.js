...
export const RELEASE_NOTES = [
  {
    ...
    body: '予定の保存を二重にして、端末の都合で消えてしまわないようにしました。保存できなかったときは画面でお知らせします。新しい予定の種類は「用事」から始まるようにし、種類ごとに時間帯の既定を持たせました（バイトなら17時から）。あわせて、はじめて開いたときの案内を作り直しました。3枚とも、紙に字が現れる形にしています。1枚目は点線の予定をタップしないと先へ進めません（このアプリで一番覚えてほしい操作なので）。2枚目で空き状況とシェアを紹介します。時給を聞く画面はやめました（時給はバイト先を作るときに決めます）。まだ予定が1件も無いときの案内が、カレンダーの最終週を隠していたのも直しています。',
  },
  {
    version: '0.15.0',
